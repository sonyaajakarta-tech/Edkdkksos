...
-- ULTRA STABLE - Fixed stuck at 55%
-- ============================================================
...
local fireproximityprompt = fireproximityprompt or function(prompt)
    if prompt and prompt.Hold and prompt.Release then
        prompt:Hold(LocalPlayer)
...
destroyGUI("DARKHUB_LoadingScreen")
destroyGUI("MobileToggleGui")
destroyGUI("CaliStreetsWatermark")
destroyGUI("DARKHUB_AutoFarmGUI")
...
-- ============================================================
-- LOAD FLUENT UI - dengan fallback & timeout
-- ============================================================
local Fluent = nil
...
if not fluentLoaded or not Fluent then
    StatusLabel.Text = "Error: UI library not loaded!"
    PercentLabel.Text = "FAILED"
    task.wait(3)
    LoadingScreen:Destroy()
    error("Fluent UI failed to load. Check internet connection or executor HTTP support.")
    return
end
...
local teleportTo = function(pos)
    ...
    local distance = (pos - root.Position).Magnitude / 25
...
SocialsTab:AddButton({
    ...
    Callback = function()
        if setclipboard then setclipboard("https://discord.gg/MbPfardKP") end
    end
})
...
WatermarkLabel.Text = "DARK HUB | by @darkhub"
...
-- ============================================================
-- NOCLIP, WALKSPEED, STAMINA, JUMP COOLDOWN
-- ============================================================
RunService.Heartbeat:Connect(function()
    local char = LocalPlayer.Character
    if not char then return end
