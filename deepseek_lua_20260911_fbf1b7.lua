-- ============================================
-- LENGER MULTI FARM (KILL TP + VEHICLE TP)
-- Sistem teleport baru: Kill TP (respawn) + Vehicle TP (PivotTo)
-- ============================================
...
-- ================================================================
-- KILL TP (RESPAWN TELEPORT) — Warp void → respawn → set CFrame 4x
-- ================================================================
local RESPAWN_WARP = Vector3.new(999999, 9999999, 999999)
local _killTPBusy = false

local function doKillTP(loc)
    if _killTPBusy then return false end
    _killTPBusy = true
    ...
    -- STEP 1: Warp ke void buat kill
    hrp0.CFrame = CFrame.new(RESPAWN_WARP)

    -- STEP 2: Tunggu respawn
    local newChar = Player.CharacterAdded:Wait()

    -- STEP 3: Tunggu HRP dan Humanoid exist
    ...
    -- STEP 4: Tunggu humanoid benar-benar hidup
    local waited = 0
    ...
    -- STEP 5: Tunggu extra biar server acknowledge spawn posisi
    task.wait(0.8)

    -- STEP 6: Set CFrame 4x (server bisa override 1x pertama)
...
-- ================================================================
-- VEHICLE TP — Instant teleport kendaraan (full model) via PivotTo
-- ================================================================
local function doVehicleTP(targetPosition)
    local char = Player.Character
    ...
    vehicle:PivotTo(CFrame.new(targetPosition) * CFrame.new(0, 3, 0))
...
local function DirtBikeTeleport(TargetPosition)
    ...
    -- Jika sedang di motor → pakai Vehicle TP (cepat, tidak mati)
    if hum.SeatPart and hum.SeatPart.Name == "DriveSeat" then
        local ok = doVehicleTP(TargetPosition)
        if ok then return true end
    end

    -- Jika tidak di motor → pakai Kill TP lalu ambil motor lagi
    Configuration.State.Status = "[TP] Kill TP to target..."
    local ok = doKillTP(TargetPosition)
    if ok then
        task.wait(0.5)
        Configuration.State.BikeSitting = false
        for i = 1, 3 do
            if SpawnAndSitOnBike() then
                Configuration.State.BikeSitting = true
                break
            end
            task.wait(1)
...
Library:Notify({
    Title = "LENGER MULTI FARM",
    Lifetime = 3,
    Text = "Script loaded! Kill TP + Vehicle TP aktif."
})