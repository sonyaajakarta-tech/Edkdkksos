...
do
    ...
    LoadingGui.Name = "dark_load_" .. HttpService:GenerateGUID(false)
...
function Library:CreateWindow(options)
    ...
    local PopupTitle = Create("TextLabel", {Parent = PopupCard, Text = "exit Dark", Font = Enum.Font.GothamBold, TextSize = 18, TextColor3 = TextColor, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 30), Position = UDim2.new(0, 0, 0, 25), ZIndex = 102, TextTransparency = 1, TextXAlignment = Enum.TextXAlignment.Center})
...
-- // CREATE WINDOW & INSTANTIATE TABS
local Window = Library:CreateWindow({Title = "Dark hub", Subtitle = "South Bronx: The Trenches", SphereText = true, SphereWords = "open"})

-- // WHITELIST WINDOW (FIXED SCROLLING ISSUE)
...
local WLStroke = Create("UIStroke", {Parent = WhitelistCard, Color = AccentColor, Thickness = 1})
local WLScale = Create("UIScale", {Parent = WhitelistCard, Scale = 0.76})

local WLHeader = Create("Frame", {Parent = WhitelistCard, BackgroundColor3 = CardColor, Size = UDim2.new(1, 0, 0, 45), ZIndex = 201})
Create("UICorner", {Parent = WLHeader, CornerRadius = UDim.new(0, 10)})
MakeDraggable(WLHeader, WhitelistCard)

local WLHeaderTitle = Create("TextLabel", {Parent = WLHeader, Text = "whitelist", Font = Enum.Font.GothamBold, TextSize = 14, TextColor3 = TextColor, BackgroundTransparency = 1, Position = UDim2.new(0, 15, 0, 0), Size = UDim2.new(1, -60, 1, 0), TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 202})
local WLCloseBtn = Create("TextButton", {Parent = WLHeader, Text = "×", Font = Enum.Font.GothamBold, TextSize = 18, TextColor3 = SubTextColor, BackgroundTransparency = 1, Size = UDim2.new(0, 35, 1, 0), Position = UDim2.new(1, -38, 0, 0), ZIndex = 202})

WLCloseBtn.MouseEnter:Connect(function() Tween(WLCloseBtn, {TextColor3 = Color3.fromRGB(255, 70, 70)}, 0.2) end)
WLCloseBtn.MouseLeave:Connect(function() Tween(WLCloseBtn, {TextColor3 = SubTextColor}, 0.2) end)

local WLSearchBox = Create("TextBox", {Parent = WhitelistCard, BackgroundColor3 = CardColor, Size = UDim2.new(1, -20, 0, 30), Position = UDim2.new(0, 10, 0, 52), Font = Enum.Font.Gotham, TextSize = 12, TextColor3 = TextColor, PlaceholderText = "Search player...", TextXAlignment = Enum.TextXAlignment.Left, ClearTextOnFocus = false, ZIndex = 201})
...
Create("UIStroke", {Parent = WLSearchBox, Color = Color3.fromRGB(45, 45, 50), Thickness = 1})
...
local WLContainer = Create("ScrollingFrame", {
    ...
    AutomaticCanvasSize = Enum.AutomaticSize.Y -- Ini penyelesaian masalah scroll
})
local WLListLayout = Create("UIListLayout", {Parent = WLContainer, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 8)})
...
local function PopulateWhitelist()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        local ItemFrame = Create("Frame", {Parent = WLContainer, BackgroundColor3 = CardColor, Size = UDim2.new(1, -5, 0, 50), ZIndex = 202})
        Create("UICorner", {Parent = ItemFrame, CornerRadius = UDim.new(0, 6)})
        Create("UIStroke", {Parent = ItemFrame, Color = Color3.fromRGB(35, 35, 40), Thickness = 1})

        local AvatarImg = Create("ImageLabel", {
            Parent = ItemFrame, BackgroundTransparency = 1, Size = UDim2.new(0, 38, 0, 38), Position = UDim2.new(0, 6, 0.5, -19),
            Image = "rbxthumb://type=AvatarHeadShot&id=" .. plr.UserId .. "&w=150&h=150", ZIndex = 203
        })
        Create("UICorner", {Parent = AvatarImg, CornerRadius = UDim.new(1, 0)})
        Create("UIStroke", {Parent = AvatarImg, Color = AccentColor, Thickness = 1})

        local NameLabel = Create("TextLabel", {
            Parent = ItemFrame, BackgroundTransparency = 1, Text = plr.DisplayName .. "\n@" .. plr.Name,
            Font = Enum.Font.Gotham, TextSize = 11, TextColor3 = TextColor, TextXAlignment = Enum.TextXAlignment.Left,
            Position = UDim2.new(0, 52, 0, 0), Size = UDim2.new(1, -100, 1, 0), TextWrapped = true, ZIndex = 203
        })

        local ToggleBtn = Create("TextButton", {
            Parent = ItemFrame, Text = isWL and "✓" or "", Font = Enum.Font.GothamBold, TextSize = 14,
            ...
            Size = UDim2.new(0, 32, 0, 32), Position = UDim2.new(1, -40, 0.5, -16), AutoButtonColor = false, ZIndex = 204
        })
    ...
    if playerCount == 0 then
        Create("TextLabel", {
            Parent = WLContainer, BackgroundTransparency = 1, Text = "No other players found.",
            Font = Enum.Font.Gotham, TextSize = 12, TextColor3 = SubTextColor, Size = UDim2.new(1, 0, 0, 50),
...
local function OpenWhitelistWindow()
    ...
    Tween(WLScale, {Scale = 0.76}, 0.3)
end

Players.PlayerAdded:Connect(function() if WhitelistCard.Visible then PopulateWhitelist() end end)
Players.PlayerRemoving:Connect(function() if WhitelistCard.Visible then PopulateWhitelist() end end)

-- // COMBAT TAB
local CombatTab = Window:CreateTab("Combat", true)
local CombatPage = CombatTab:CreatePage("Combat")

local CombatSection = CombatPage:CreateSection("Combat")
CombatSection:AddToggleWithBind("Enable Silent Aim", CFG.ENABLED, Enum.KeyCode.K, function(state) CFG.ENABLED = state end)
CombatSection:AddToggleWithBind("Wallbang", CFG.WALLBANG, Enum.KeyCode.L, function(state) CFG.WALLBANG = state end)

-- Noclip masuk ke sini (di bawah Wallbang)
CombatSection:AddToggleWithBind("Noclip", false, Enum.KeyCode.J, function(state)
...
CombatSection:AddButton("Manage Whitelist", function() OpenWhitelistWindow() end)

local VisualSection = CombatPage:CreateSection("Visual")
VisualSection:AddToggle("Show FOV Circle", CFG.SHOW_FOV, function(state) CFG.SHOW_FOV = state end)
VisualSection:AddToggleWithColor("Tracer", CFG.SHOW_TRACER, CFG.TRACER_COLOR, function(state) CFG.SHOW_TRACER = state end, function(color) CFG.TRACER_COLOR = color end)
VisualSection:AddSlider("FOV Radius", 10, 800, CFG.FOV_RADIUS, function(val) CFG.FOV_RADIUS = val end)
...
ThemeSection:AddKeybind("ui", Enum.KeyCode.RightAlt, function()
    Window:ToggleUI()
end)
