...
local Library = { } do
    ...
    Library.Window = function(Self, Params)
        ...
        Library.HideUsername = false

        function Window:SetUsernameHidden(Bool)
            Library.HideUsername = Bool and true or false
            Items.Username.Instance.Text = Library.HideUsername and "Havoc" or LocalPlayer.DisplayName
        end
        ...
        local ProfilePanel = MakeFrame({
            Parent = Library.UnusedHolder.Instance,
            Size = UDim2.fromOffset(ProfileW, 310),
            Color = "Section",
            Round = 10,
            Z = 45
        })
        ...
        MakeText({
            ...
            TextSize = 14,
            Anchor = Vector2.new(0, 0.5),
            Pos = UDim2.new(0, 14, 0, 154),
            Size = UDim2.fromOffset(120, 20),
            Color = "DimText",
            Z = 46
        })

        local HideBox = MakeFrame({
            Parent = ProfilePanel.Instance,
            Anchor = Vector2.new(1, 0.5),
            Pos = UDim2.new(1, -14, 0, 154),
        ...
        local HideCircle = MakeFrame({
            Parent = HideBox.Instance,
            Anchor = Vector2.new(0, 0.5),
        ...
        local HideHit = MakeButton({
            Parent = ProfilePanel.Instance,
            Anchor = Vector2.new(1, 0.5),
            Pos = UDim2.new(1, -8, 0, 154),
        ...
        local function SetHide(State)
            ...
            local BoxKey = State and "Light" or "Element"
            local CircleKey = State and "Accent" or "DimText"

            HideBox:ChangeItemTheme({ BackgroundColor3 = BoxKey })
            HideCircle:ChangeItemTheme({ BackgroundColor3 = CircleKey })

            Library:Tween({ BackgroundColor3 = Library.Theme[BoxKey] }, nil, HideBox.Instance)
            Library:Tween({ BackgroundColor3 = Library.Theme[CircleKey] }, nil, HideCircle.Instance)
            Library:Tween({
                Position = State and UDim2.new(0, 18, 0.5, 0) or UDim2.new(0, 4, 0.5, 0)
            }, nil, HideCircle.Instance)

            Window:SetUsernameHidden(State)
        end

        HideHit:Connect("MouseButton1Down", function()
            SetHide(not HideState)
        end)
