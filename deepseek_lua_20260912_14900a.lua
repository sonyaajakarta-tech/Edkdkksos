-- ================================================================
-- DARK HUB PREMIUM V3.3 — ZOLAR UI EDITION
-- UI: Zolar • Logic: Original (Unchanged)
-- TP: ONLY "Others" category (ATM/Homeless/Apartment removed)
-- Teleport: ONLY Kill TP. VEH/Normal/Player TP removed.
-- ================================================================
...
local ovLogoFrame = Instance.new("Frame", ovCenter)
ovLogoFrame.Size = UDim2.new(0, 0, 0, 72)
ovLogoFrame.AutomaticSize = Enum.AutomaticSize.X
ovLogoFrame.AnchorPoint = Vector2.new(0.5, 0)
ovLogoFrame.Position = UDim2.new(0.5, 0, 0, 0)
ovLogoFrame.BackgroundTransparency = 1
ovLogoFrame.ZIndex = 10001
local _ol = Instance.new("UIListLayout", ovLogoFrame)
_ol.FillDirection = Enum.FillDirection.Horizontal
_ol.SortOrder = Enum.SortOrder.LayoutOrder
_ol.VerticalAlignment = Enum.VerticalAlignment.Center

local ovTitleGreen = Instance.new("TextLabel", ovLogoFrame)
...
local ovTitleWhite = Instance.new("TextLabel", ovLogoFrame)
...
local ovWS = Instance.new("UIStroke", ovTitleWhite)
ovWS.Color = Color3.fromRGB(0, 170, 255)
ovWS.Thickness = 2.5
...
local function openBonusTPOverlay(destName)
    ...
    showTPOverlay(destName, 0)
end
local function updateBonusTPLabel(destName)
    ovLabel.Text = "TELEPORTING TO " .. destName
    ovLabel.TextColor3 = Color3.fromRGB(160, 180, 210)
end
...
local modifiedParts = {}
local ghostConn = nil
local OwnedKitchenPos, OwnedDoorPos = nil, nil
local mBags = {
...
local function startGhostMode()
    ...
    ghostConn = RunService.Heartbeat:Connect(function()
        ...
        local function proc(part)
            ...
            if m and (m:FindFirstChildOfClass("VehicleSeat", true) or m:FindFirstChildOfClass("Seat", true)) then return end
        ...
        local dirs = { Vector3.new(0,-4,0), hrp.CFrame.LookVector*3.5, -hrp.CFrame.LookVector*3.5,
...
local function countTool(nameOrList)
    ...
    checkContainer(char); checkContainer(backpack)
    return count
end
local function hasTool(t) return countTool(t) > 0 end
...
local function secureDoor(doorPos)
    for i = 1, 10 do
        ...
        if checkPromptExistsAt(doorPos, 8, "lock") then
            ...
            firePromptAt(doorPos, 8, "open"); task.wait(0.5)
        end
    end
    return false
end
...
local function robustPutIngredient(toolNameOrList, waitTimeAfter)
    ...
    if countTool(toolNameOrList) < initialCount then
        if waitTimeAfter and waitTimeAfter > 0 then
            local ws = os.clock()
            while FarmActive and (os.clock() - ws) < waitTimeAfter do
                if checkDeathStatus() then return false end
                task.wait(0.5)
            end
        end
...
local function RunFarm()
    ...
    while FarmActive and Running do
        if isFirstRun then BypassTP(shopPos); isFirstRun = false end
        robustBuy()
...
local function startFarm()
    ...
    task.spawn(function()
        if not OwnedKitchenPos then
            ...
            if not ok then FarmActive = false; return end
        end
        RunFarm()
...
FNL:Textbox({ Name="In-Game Name", Placeholder="name", Flag="FNGameName" })
FNL:Textbox({ Name="Username", Placeholder="user", Flag="FNUsername" })
FNL:Button({ Name="Apply", Callback=function()
    pcall(function()
        local char = plr.Character
        ...
        if tag1 then
            ...
            if lbl then lbl.Text = "RENAMED" end
        end
    end)
end })
...
local SilentAimToggle = AimR:Toggle({ Name="Silent Aim", Default=false, Flag="SilentAim",
...
ESPL:Toggle({ Name="Name", Default=true, Flag="ESPName",
    Callback=function(s) Flags.ESPName=s end })
ESPL:Toggle({ Name="Distance", Default=true, Flag="ESPDist",
    Callback=function(s) Flags.ESPDist=s end })
ESPL:Toggle({ Name="HP Bar", Default=true, Flag="ESPHPBar",
    Callback=function(s) Flags.ESPHPBar=s end })
ESPL:Toggle({ Name="GUN", Default=true, Flag="ESPWeapon",
    Callback=function(s) Flags.ESPWeapon=s end })
ESPL:Toggle({ Name="Skeleton", Default=false, Flag="ESPSkeleton",
    Callback=function(s) Flags.ESPSkeleton=s end })
ESPL:Toggle({ Name="Masak", Default=true, Flag="ESPMasak",
    Callback=function(s) Flags.ESPMasak=s end })
...
local FarmSub = FarmTab:SubTab({ Name = "Marshmallow", Icon = "list" })
local FarmL = FarmSub:Section({ Name = "Auto Farm", Side = 1 })
FarmL:Toggle({ Name="Start Auto Farm", Default=false, Flag="FarmActive",
    Callback=function(s)
        if s then startFarm() else stopFarm() end
    end })
FarmL:Slider({ Name="Mau Masak Berapa", Min=1, Max=50, Default=1, Suffix="x", Flag="FarmBatch",
    Callback=function(v) FarmBatchAmount=v end })
FarmL:Paragraph({ Title="Info", Content="Auto beli bahan → masak → jual. Cari apartment kosong otomatis. Noclip + Anti-AFK saat farm jalan." })
...
HPTLabel.Text = "BLINK"
HPTLabel.TextColor3 = Color3.fromRGB(0, 170, 255)
        ...
        if cb and cw then
            local OldCast = hookfunction(cb, function(...)
                ...
                for _, p in pairs(game.Players:GetPlayers()) do
                    ...
                    local hitPart = ch:FindFirstChild(SilentPart == "Head" and "Head" or "HumanoidRootPart")
...
RunService.RenderStepped:Connect(function()
    ...
    for p, e in pairs(ESP) do
        ...
        if Flags.ESPHPBar then
            ...
            e.hpbg.Filled=false; e.hpbg.Thickness=1; e.hpbg.Visible=true
            e.hpbar.Color=hpCol
        ...
        if Flags.ESPWeapon and wName then
            e.weapon.Text=wName
            e.weapon.Color=Color3.fromRGB(0,170,255)
...
Zolar:Notification({
    ...
    Description = "TP: Others only. Have fun!",
    Icon = "check",
    Duration = 6
})