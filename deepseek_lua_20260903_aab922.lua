-- DARK HUB LOADER - FIXED
-- Compatible with all executors (KRNL, Synapse X, ScriptWare, etc.)
-- Rebranded from "STR HUB" to "DARK HUB"
-- Discord: https://discord.gg/MbPfardKP
...
local SCRIPT_URL = "https://raw.githubusercontent.com/sonyaajakarta-tech/Edkdkksos/refs/heads/main/deepseek_lua_20260903_400a6b.lua"
local KEY_FILE = "StrixSecureKeyCache.json"
...
local function safeTween(obj, info, props)
    ...
    if success then return tween else return nil end
end

local function safeUIStroke(parent, props)
...
local function showBlockGui()
    ...
    local fade = safeTween(overlay, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
        BackgroundTransparency = 0.5
    })
    if fade then fade:Play() end
    ...
    local function closeGui()
        local fadeOut = safeTween(overlay, TweenInfo.new(0.2), {BackgroundTransparency = 1})
        if fadeOut then fadeOut:Play() end
        local shrink = safeTween(blockFrame, TweenInfo.new(0.2), {Size = UDim2.new(0, 0, 0, 0)})
...
local function showKeyGui()
    ...
    local bgTween = safeTween(mainFrame, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {
        BackgroundTransparency = 0
    })
    if bgTween then bgTween:Play() end
    ...
    local inputStroke = safeUIStroke(inputFrame, {
    ...
    keyBox.Focused:Connect(function()
        if inputStroke then
            local t = safeTween(inputStroke, TweenInfo.new(0.2), {
                Color = Color3.fromRGB(255, 255, 255)
            })
    ...
    rememberFrame.MouseButton1Click:Connect(function()
        ...
        local t = safeTween(checkMark, TweenInfo.new(0.15), {
        ...
        local t2 = safeTween(checkBox, TweenInfo.new(0.15), {
    ...
    verifyBtn.MouseButton1Click:Connect(function()
        if keyBox.Text == TARGET_KEY then
            ...
            local flash = safeTween(verifyBtn, TweenInfo.new(0.2), {
                BackgroundColor3 = Color3.fromRGB(200, 200, 200)
            })
            if flash then flash:Play() end

            waitTime(0.6)

            local fadeOut = safeTween(mainFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            ...
            -- Load payload with error handling
            local success, err = pcall(function()
                ...
                if fn then
                    ...
                    warn("Failed to load payload: invalid Lua code")
                end
            end)
            if not success then
...
if saved and saved.Key == TARGET_KEY and saved.Remember then
    local success, err = pcall(function()
        ...
        if fn then
            ...
            warn("Auto-login failed: invalid payload")
        end
    end)
    if not success then
        warn("Target script execution failed: " .. tostring(err))
        -- Fallback to key GUI if auto-login fails
        showKeyGui()
    end
    return
end
